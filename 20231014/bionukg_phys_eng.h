...
namespace bionukg
{
    ...
    namespace phy
    {
        ...
        private:
            ...
            public:
                // F=-k*dx
                virtual vector_<N, dataType> force(Particle<N, dataType> *thisp) const override
                {
            ...
            virtual dataType energy() const override
            {
        ...
        template <int N = 2, typename dataType = double>
        class callback_ : public interaction_between<N, dataType>
        {
        private:
            vector_<N, dataType> (*cbk_force)() = 0;
        ...
        public:
            ...
            void init(Particle<N, dataType>* p1,
                vector_<N, dataType>(*callbk)())
            {
                this->p1 = p1;
                p1->particle_interact_list.add(this);
                cbk_force = callbk;
            }
            void init(Particle<N, dataType>* p1, const callback_& another)
            {
                init(p1, another.cbk_force);
            }
            ...
            callback_(vector_<N, dataType>(*callbk)())
            {
                this->p1 = 0;
                this->cbk_force = callbk;
            };

            callback_(Particle<N, dataType>* p1, vector_<N, dataType>(*callbk)())
            {
                init(p1, callbk);
            };
            callback_(Particle<N, dataType>* p1, const callback_& another)
            {
                init(p1, another);
            }
            ~callback_()
